...
export async function dispatch(env:SchedulerEnv, fetcher:DispatchFetch = fetch) {
  ...
  const endpoint = `https://api.github.com/repos/${env.GITHUB_REPOSITORY}/actions/workflows/collect-firefox.yml/dispatches`;
  let response;
  try {
    response = await fetcher(endpoint, {
      ...
      headers:{
        ...
        'User-Agent':'uk-cinema-firefox-scheduler',
  ...
  return {event:'workflow-dispatched',status:response.status,workflow:'collect-firefox.yml',deliveryEnabled:false};
}
export default {
