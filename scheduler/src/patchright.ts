...
export async function dispatch(env:SchedulerEnv, fetcher:DispatchFetch = fetch) {
  ...
  const endpoint = `https://api.github.com/repos/${env.GITHUB_REPOSITORY}/actions/workflows/collect-patchright.yml/dispatches`;
  let response;
  try {
    response = await fetcher(endpoint, {
      ...
      headers:{
        ...
        'User-Agent':'uk-cinema-patchright-scheduler',
  ...
  return {event:'workflow-dispatched',status:response.status,workflow:'collect-patchright.yml',deliveryEnabled:false};
}
export default {
