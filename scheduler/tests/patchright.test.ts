...
import worker,{dispatch} from '../src/patchright.ts';
...
test('dispatches only the configured workflow with delivery disabled',async()=>{
 ...
 const r=await dispatch({...env,DELIVER:'true'},async(url,options)=>{requests.push({url,options});return new Response(null,{status:204});});
 const request=requests[0];
 assert.ok(request);
 assert.match(request.url,/example\/collector\/actions\/workflows\/collect-patchright.yml\/dispatches$/);
 assert.deepEqual(JSON.parse(String(request.options.body)),{ref:'main'});
 assert.equal(r.event,'workflow-dispatched');assert.equal(r.deliveryEnabled,false);assert.ok(!JSON.stringify(r).includes('test-token'));
});
