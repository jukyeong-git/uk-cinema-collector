...
export function responseDiagnostic(response: DiagnosticResponse | null | undefined) {
  if (!response) return {responseReceived:false as const};
  const headers=response.headers();
  const status=response.status();
  const mime=(headers['content-type'] ?? '').split(';')[0].trim().toLowerCase();
  const ray=headers['cf-ray'] ?? '';
  return {
    responseReceived:true as const,httpStatus:status,
...
export function errorDiagnostic(value: unknown) {
