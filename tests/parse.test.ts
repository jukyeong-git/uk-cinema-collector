...
import {localTime,parsePage,checkedPageUrl} from '../src/parse.ts';
const base='https://whatson.bfi.org.uk/imax/Online/default.asp';
const id='00000000-0000-4000-8000-000000000001';
const article='00000000-0000-4000-8000-000000000002';
function page({number=1,total=1,next=null,status='available',date='Friday 18 September 2026 21:00'}: {number?: number; total?: number; next?: string | null; status?: string; date?: string}={}) {
...
test('follows only the expected next page',()=>{const next='default.asp?BOset::WScontent::SearchResultsInfo::current_page=2';const r=parsePage(page({total:2,next}),base,1);assert.equal(r.totalPages,2);assert.ok(r.next);assert.ok(r.next.includes('current_page=2'));});
